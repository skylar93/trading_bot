...
class MultiAgentManager:
    ...
    def _calculate_position_sizes(self, actions: Dict[str, np.ndarray], market_state: Dict[str, Any]) -> Dict[str, float]:
        """Calculate position sizes for each agent while respecting total exposure limits."""
        total_exposure = 0.0
        position_sizes = {}
        
        # Calculate trend strength
        trend_strength = self._calculate_trend_strength(market_state)
        
        # First pass: Calculate initial position sizes
        for agent_id, action in actions.items():
            agent = self.agents[agent_id]
            
            # Base position size on action
            position_size = float(action)
            
            # Scale position based on market regime
            if hasattr(agent, 'strategy'):
                if agent.strategy == "momentum" and trend_strength < 0.6:
                    position_size *= 0.2  # Reduce momentum positions in weak trends
                elif agent.strategy == "mean_reversion" and trend_strength > 0.6:
                    position_size *= 0.2  # Reduce mean reversion positions in strong trends
            
            position_sizes[agent_id] = position_size
            total_exposure += abs(position_size)
        
        # Second pass: Scale positions if total exposure exceeds limit
        if total_exposure > 1.5:  # Maximum allowed total exposure
            scaling_factor = 1.5 / total_exposure
            for agent_id in position_sizes:
                position_sizes[agent_id] *= scaling_factor
        
        return position_sizes
    
    def _calculate_trend_strength(self, market_state: Dict[str, Any]) -> float:
        """Calculate the strength of the current market trend."""
        if 'close' not in market_state:
            return 0.5
        
        prices = market_state['close']
        if len(prices) < 10:
            return 0.5
        
        # Calculate trend strength based on price changes
        price_changes = np.diff(prices[-10:])
        trend_persistence = np.sum(np.sign(price_changes)) / len(price_changes)
        
        return float(np.clip(abs(trend_persistence), 0.0, 1.0))
