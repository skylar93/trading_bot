...
class SingleAssetRLTradingEnv(gym.Env):
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if start_idx > 0:
            ...
            ref_close = float(close[0])  # self-reference → log-return[0] = 0

        # Build prev_close array: [ref_close, close[0], close[1], ..., close[-2]]
        prev_close = np.empty(self.window_size, dtype=np.float64)
        prev_close[0] = ref_close
        if self.window_size > 1:
            prev_close[1:] = close[:-1]

        # Log-relatives (all vs previous close)
        log_open  = np.log(np.maximum(open_  / (prev_close + eps), eps))
        log_high  = np.log(np.maximum(high   / (prev_close + eps), eps))
        log_low   = np.log(np.maximum(low    / (prev_close + eps), eps))
        log_close = np.log(np.maximum(close  / (prev_close + eps), eps))

        # Volume: log-normalized by window mean
        vol_mean = np.mean(vol) + eps
        log_vol  = np.log(vol / vol_mean + eps)
