...
class Order:
    def __init__(
        ...
        if order_type == OrderType.TRAILING_STOP:
            ...
            self.trailing_price = (
                stop_price if stop_price else price if price else None
            )
        ...
        # For iceberg orders
        self.visible_qty = (
            min(iceberg_qty, quantity) if iceberg_qty else quantity
        )
        self.executed_qty = 0
...
class PaperTradingEnvironment:
    ...
    async def update_market_data(self, data: Dict):
        ...
        if "type" in data and data["type"] == "ticker":
            ...
            if self.position != 0 and self.stop_loss_price != 0:
                if (
                    self.position > 0
                    and self._latest_price <= self.stop_loss_price
                ) or (
                    self.position < 0
                    and self._latest_price >= self.stop_loss_price
                ):
                ...
                if order.order_type == OrderType.TRAILING_STOP:
                    if order.side == "sell":
                        if self._latest_price > order.trailing_price:
                            ...
                            self.logger.debug(
                                f"Updated sell trailing price to {order.trailing_price}"
                            )
                    else:  # buy
                        if self._latest_price < order.trailing_price:
                            order.trailing_price = self._latest_price
                            self.logger.debug(
                                f"Updated buy trailing price to {order.trailing_price}"
                            )
            ...
            if order.order_type == OrderType.MARKET:
                ...
                if order.side == "buy":
                    ...
                    if cost <= self.balance:
                        ...
                        return {
                            "success": True,
                            "message": f"Market buy executed at {price}",
                        }
                else:  # sell
                    if order.quantity <= self.position:
                        revenue = (
                            order.quantity * price * (1 - self.trading_fee)
                        )
                        ...
                        return {
                            "success": True,
                            "message": f"Market sell executed at {price}",
                        }
                return {
                    "success": False,
                    "message": "Insufficient balance/position",
                }
            else:
                ...
                if order.order_type == OrderType.TRAILING_STOP:
                    order.trailing_price = self._latest_price
                    self.logger.info(
                        f"Initialized trailing price to {order.trailing_price}"
                    )
                ...
                return {
                    "success": True,
                    "message": f"Order placed: {order.order_type.value}",
                }
        ...
        for order in self.active_orders[:]:
            ...
            if order.order_type == OrderType.LIMIT:
                ...
                if order.side == "buy" and self._latest_price <= order.price:
                    execute_order = True
                    self.logger.info(
                        f"Buy limit order executed at {self._latest_price}"
                    )
                # For sell limit orders, execute when price rises above limit price
                elif (
                    order.side == "sell" and self._latest_price >= order.price
                ):
                    execute_order = True
                    self.logger.info(
                        f"Sell limit order executed at {self._latest_price}"
                    )
                ...
                if order.side == "sell":
                    if (
                        not order.triggered
                        and self._latest_price >= order.stop_price
                    ):
                        order.triggered = True
                        self.logger.info(
                            f"Sell stop-limit order triggered at {self._latest_price}"
                        )
                    elif order.triggered and self._latest_price >= order.price:
                        execute_order = True
                        self.logger.info(
                            f"Sell stop-limit order executed at {self._latest_price}"
                        )
                # For buy stop-limit:
                # 1. Trigger when price rises above stop price
                # 2. Execute when price falls to limit price after trigger
                    ...
                    if (
                        ...
                        order.triggered = True
                        self.logger.info(
                            f"Buy stop-limit order triggered at {self._latest_price}"
                        )
                    elif order.triggered and self._latest_price <= order.price:
                        execute_order = True
                        self.logger.info(
                            f"Buy stop-limit order executed at {self._latest_price}"
                        )
                ...
                if order.side == "sell":
                    ...
                    # Execute when price falls below trailing price by trailing percentage
                        ...
                        1 - order.trailing_pct
                    ):
                        execute_order = True
                        self.logger.info(
                            f"Trailing stop sell executed at {self._latest_price}"
                        )
                else:  # buy
                    ...
                    # Execute when price rises above trailing price by trailing percentage
                        ...
                        execute_order = True
                        self.logger.info(
                            f"Trailing stop buy executed at {self._latest_price}"
                        )
                ...
                if order.side == "buy" and self._latest_price <= order.price:
                    execute_order = True
                    execute_quantity = min(
                        order.iceberg_qty, order.quantity - order.executed_qty
                    )
                    self.logger.info(
                        f"Buy iceberg order partial execution: {execute_quantity} at {self._latest_price}"
                    )
                # For sell iceberg orders, execute when price rises above limit price
                    ...
                    self.logger.info(
                        f"Sell iceberg order partial execution: {execute_quantity} at {self._latest_price}"
                    )
            ...
            if execute_order:
                try:
                    ...
                    self.logger.info(
                        f"Order executed: {order.order_type.value} {order.side} at {self._latest_price}"
                    )
                except Exception as e:
        ...
        try:
            ...
            if order.side == "buy":
                ...
                if cost <= self.balance:
                    ...
                    self.filled_orders.append(order)
                    self.logger.info(
                        f"Order filled: {order.order_type.value} buy {order.quantity} @ {price}"
                    )
                else:
                    self.logger.warning(
                ...
                if order.quantity <= self.position:
                    ...
                    self.filled_orders.append(order)
                    self.logger.info(
                        f"Order filled: {order.order_type.value} sell {order.quantity} @ {price}"
                    )
                else:
                    self.logger.warning(
                        f"Insufficient position for sell order: {order.quantity} > {self.position}"
                    )
        except Exception as e:
    ...
    async def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "unrealized_pnl": (
                (self._latest_price - self.entry_price) * self.position
                if self.position != 0
                else 0
            ),
    ...
    async def check_expired_orders(self):
        ...
        for order in self.active_orders[
            :
        ]:  # Use slice copy to safely modify list during iteration
    ...
    async def cancel_order(self, order_id: str) -> Dict:
        ...
        return {
            "success": False,
            "message": f"Order {order_id} not found or already processed",
        }
    ...
    async def execute_order(self, size_fraction: float) -> Dict:
        ...
        if size_fraction > 0:  # Buy
            ...
            order = Order(
                order_type=OrderType.MARKET, side="buy", quantity=quantity
            )
        else:  # Sell
            if self.position == 0:
            ...
            order = Order(
                order_type=OrderType.MARKET, side="sell", quantity=quantity
            )
