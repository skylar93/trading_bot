...
class LiveTradingEnvironment(gym.Env):
    ...
    def step(
        ...
        reward = (
            new_portfolio_value - self._last_portfolio_value
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if len(df) < self.window_size:
            logger.warning(
                f"Not enough data points: {len(df)} < {self.window_size}"
            )
        ...
        features.append(
            pd.Series([best_bid / price_mean if best_bid else 0] * len(df))
        )
        features.append(
            pd.Series([best_ask / price_mean if best_ask else 0] * len(df))
        )
        features.append(
            pd.Series(
                [bid_volume / volume_mean if bid_volume else 0] * len(df)
            )
        )
        features.append(
            pd.Series(
                [ask_volume / volume_mean if ask_volume else 0] * len(df)
            )
        )
        ...
        features.append(
            pd.Series([self.balance / self.initial_balance] * len(df))
        )
