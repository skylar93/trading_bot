...
def main():
    ...
    experiment_name = st.sidebar.text_input("MLflow Experiment Name", "trading_bot")
    ...
    with col1:
        ...
        if experiment:
            ...
            if runs:
                ...
                portfolio_values = [m.value for m in client.get_metric_history(latest_run.info.run_id, "portfolio_value")]
                fig.add_trace(
                ...
                actions = [m.value for m in client.get_metric_history(latest_run.info.run_id, "action")]
                fig.add_trace(
                    go.Scatter(y=actions, name="Trading Actions"),
                    row=2, col=1
                )
    ...
    with col2:
        ...
        trades = [m.value for m in client.get_metric_history(latest_run.info.run_id, "trades")]
        if trades:
        ...
        with col2_2:
            if len(portfolio_values) > 1:
                pct_change = (portfolio_values[-1] - portfolio_values[0]) / portfolio_values[0] * 100
