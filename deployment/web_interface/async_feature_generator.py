...
log_file = os.path.join(log_dir, f"async_feature_generation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
class AsyncFeatureGenerator:
    ...
    def get_progress(self) -> Dict[str, Any]:
        ...
        if self._start_time and self._running:
            ...
            logger.debug(f"Progress: {progress*100:.1f}% - Elapsed: {elapsed:.1f}s - ETA: {eta:.1f}s")
    ...
    def start(self, data: pd.DataFrame):
        ...
        logger.info(f"Starting feature generation for {len(data)} rows of data")
        logger.debug(f"Data columns: {data.columns.tolist()}")
        logger.debug(f"Data sample:\n{data.head()}")
        
        self._running = True
        self._progress = {'progress': 0.0, 'message': 'Starting feature generation...'}
        self._result = None
        self._start_time = time.time()
        
        def _run_generation():
            try:
                logger.info("Starting feature generation process in background thread")
                ...
                logger.info(f"Memory usage before generation: {mem_before:.1f} MB")
                ...
                logger.info(f"Memory usage after generation: {mem_after:.1f} MB")
                
                elapsed = time.time() - start_time
                logger.info(f"Feature generation completed successfully in {elapsed:.2f} seconds")
                logger.debug(f"Generated features: {result.columns.tolist()}")
                self._result = ('success', result)
                
            except Exception as e:
                elapsed = time.time() - start_time
                logger.error(f"Error in feature generation after {elapsed:.2f} seconds: {str(e)}", exc_info=True)
    ...
    def stop(self):
        ...
        if self._start_time:
            elapsed = time.time() - self._start_time
            logger.info(f"Feature generation stopped after {elapsed:.2f} seconds")