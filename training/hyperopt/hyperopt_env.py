    ...
    def step(
        ...
        if abs(action) > 0.05:  # Threshold to prevent tiny trades
            if action > 0:  # Buy
                ...
                revenue = (
                    shares_to_sell * current_price * (1 - self.trading_fee)
                )
        ...
        recent_returns = (
            self.returns[-self.window_size :]
            if len(self.returns) > self.window_size
            else self.returns
        )
        reward = (
            self._calculate_sharpe_ratio(recent_returns)
            if recent_returns
            else 0
        )
