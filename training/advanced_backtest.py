...
class ScenarioBacktester(Backtester):
    ...
    def run_low_liquidity_scenario(
        self, agent: Any, window_size: int = 20, verbose: bool = True
    ) -> Dict:
    ...
    def save_scenario_results(
        self, results: Dict, scenario_type: str, save_dir: str
    ):
        ...
        scenario_df.to_csv(
            save_dir / f"{scenario_type}_metrics.csv", index=False
        )
