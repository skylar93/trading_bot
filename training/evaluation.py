...
class TradingMetrics:
    ...
    def calculate_returns(portfolio_values: np.ndarray) -> np.ndarray:
        ...
        portfolio_values = np.array(portfolio_values).reshape(
            -1
        )  # Ensure 1D array
    ...
    def evaluate_strategy(
        ...
        max_drawdown, dd_start, dd_end = (
            TradingMetrics.calculate_maximum_drawdown(portfolio_values)
        )
        ...
        metrics = {
            ...
            * 100,
            "sharpe_ratio": TradingMetrics.calculate_sharpe_ratio(
                returns, risk_free_rate
            ),
            "sortino_ratio": TradingMetrics.calculate_sortino_ratio(
                returns, risk_free_rate
            ),
...
def calculate_metrics(
    portfolio_values: List[float], returns: List[float]
) -> Dict[str, float]:
    ...
    portfolio_values = np.nan_to_num(
        portfolio_values, nan=0.0, posinf=0.0, neginf=0.0
    )
...
def calculate_trade_metrics(trades: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    profit_factor = (
        sum(profits) / (sum(losses) + 1e-6) if losses else float("inf")
    )
