...
class MLflowManager:
    ...
    def __init__(
        self, experiment_name: str, tracking_dir: Optional[str] = None
    ):
    ...
    def log_dataframe(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                mlflow.log_artifact(
                    f.name, os.path.join(artifact_path, filename)
                )
                os.remove(f.name)
    ...
    def log_backtest_results(
        self, df_results: pd.DataFrame, metrics: dict, trades: list
    ):
