...
class ScenarioGenerator:
    ...
    def generate_flash_crash(self, 
        ...
        # Generate OHLCV data
        df = pd.DataFrame({
            'datetime': pd.date_range(start='2024-01-01', periods=length, freq='1min'),
            '$open': prices,
    ...
    def generate_choppy_market(self,
        ...
        trend = self.base_price + np.sin(t) * self.base_price * 0.1 * chop_intensity
        noise = np.random.normal(0, self.volatility * chop_intensity * 2, length)
        prices = trend + noise
...
class ScenarioTester:
    ...
    def test_scenario(self, data: pd.DataFrame, scenario_type: str = 'normal') -> Dict[str, Any]:
    ...
    def _test_flash_crash(self) -> Dict[str, Any]:
        ...
        return {
            'metrics': {
                ...
                'max_drawdown': (max(portfolio_values[:max_drawdown_idx]) - min(portfolio_values[max_drawdown_idx:])) / max(portfolio_values[:max_drawdown_idx]),
    ...
    def _test_low_liquidity(self) -> Dict[str, Any]:
        ...
        return {
            'metrics': {
                ...
                'trade_count_low_liq': len([t for t in trades if 300 <= timestamps.index(t['entry_time']) < 400])
            },
            'trades': trades,
            'portfolio_values': portfolio_values,
            'timestamps': timestamps
    ...
    def _test_mean_reversion(self) -> Dict[str, Any]:
        ...
        mr_strength = self._calculate_mean_reversion_strength(positions, prices)
        
        return {
            'mean_reversion_strength': mr_strength,
            'position_changes': len([i for i in range(1, len(positions)) if positions[i] != positions[i-1]])
        }
    ...
    def _test_normal(self) -> Dict[str, Any]:
        ...
        sharpe = np.sqrt(252) * returns.mean() / volatility if volatility > 0 else 0
        ...
        winning_trades = sum(1 for t in trades if 
                           (t['type'] == 'buy' and t['price'] < portfolio_values[-1]) or
                           (t['type'] == 'sell' and t['price'] > portfolio_values[-1]))
    ...
    def _calculate_trend_strength(self, positions: List[int], returns: List[float]) -> float:
    ...
    def _calculate_mean_reversion_strength(self, positions: List[int], prices: List[float]) -> float:
        """Calculate mean reversion strength"""
