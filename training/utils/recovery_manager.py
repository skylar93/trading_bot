    ...
    def register_worker(
        ...
        with self._lock:
            ...
            self._workers[worker_id] = WorkerState(
                worker_id=worker_id,
                status='active',
                last_checkpoint=self._get_checkpoint_path(worker_id, 'initial'),
                failure_count=0,
                ...
                with self._lock:
                    for worker_id, state in self._workers.items():
                        if state.status == 'active':
                            ...
                            if current_time - state.last_heartbeat > self._heartbeat_interval * 2:
                                logger.warning(f"Worker {worker_id} missed heartbeat")
                                self.mark_worker_failed(worker_id)
                
            except Exception as e:
                logger.error(f"Error in worker monitoring: {str(e)}", exc_info=True)
