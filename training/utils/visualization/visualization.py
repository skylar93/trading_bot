...
class TradingVisualizer:
    ...
    def _add_trade_markers(
        self, ax, portfolio_values: np.ndarray, trades: List[Dict]
    ):
        ...
        for trade in trades:
            ...
            ax.scatter(
                ...
                portfolio_values[trade.get("entry_time", 0)],
                marker="^",
                color=color,
                alpha=0.6,
            )
            ax.scatter(
                ...
                portfolio_values[trade.get("exit_time", 0)],
                marker="v",
                color=color,
                alpha=0.6,
            )
    ...
    def _plot_drawdown(self, ax, portfolio_values: np.ndarray):
        ...
        ax.fill_between(
            np.arange(len(drawdown)), drawdown, 0, color="r", alpha=0.3
        )
