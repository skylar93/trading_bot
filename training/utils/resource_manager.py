            ...
            try:
                ...
                with self._lock:
                    ...
                    if len(self._stats_history) > self.history_size:
                        self._stats_history = self._stats_history[
                            -self.history_size :
                        ]

            except Exception as e:
                logger.error(
                    f"Error collecting stats: {str(e)}", exc_info=True
                )
        ...
        if self.has_gpu:
            ...
            for i in range(self.num_gpus):
                with torch.cuda.device(i):
                    memory_stats = torch.cuda.get_device_properties(
                        i
                    ).total_memory
...
class ResourceManager:
    ...
    def allocate_resources(
        ...
        memory_percent = (
            (stats.memory_used + memory_needed) / stats.memory_total * 100
        )
        ...
        if gpu_needed and stats.gpu_stats:
            ...
            for gpu in stats.gpu_stats:
                memory_percent = (
                    (gpu.memory_used + gpu_needed) / gpu.memory_total * 100
                )
        ...
        return {"allocated": True, "gpu_device": gpu_device}
    ...
    def get_resource_usage(self) -> Dict[str, float]:
        ...
        if stats.gpu_stats:
            for i, gpu in enumerate(stats.gpu_stats):
                ...
                usage[f"gpu_{i}_memory_percent"] = (
