...
class RiskAwareBacktester(Backtester):
    ...
    def execute_trade(
        ...
        if not risk_assessment["allowed"]:
            ...
            result["portfolio_value"] = self.balance + (
                self.position * price_data["$close"]
            )
            return result
        ...
        risk_adjusted_size = min(
            original_size, risk_assessment["position_size"] / portfolio_value
        )
        ...
        trade_result["portfolio_value"] = self.balance + (
            self.position * price_data["$close"]
        )
