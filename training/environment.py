...
class TradingEnvironment(gym.Env):
    ...
    def __init__(self, data: pd.DataFrame, initial_balance: float = 10000, trading_fee: float = 0.001):
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, 
            high=np.inf, 
            shape=(len(data.columns) + 4,),  # features + [balance, position, unrealized_pnl, current_price]
            dtype=np.float32
        )
    ...
    def get_state(self) -> np.ndarray:
        ...
        unrealized_pnl = (current_price - self.position_value) * self.position if self.position != 0 else 0
