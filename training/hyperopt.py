...
class HyperparameterOptimizer:
    ...
    def _train_with_params(
        self, config: Dict, train_data: pd.DataFrame, val_data: pd.DataFrame
    ):
        ...
        self.pipeline.config["model"].update(
            {
                "fcnet_hiddens": [config["hidden_size"]]
                * config["num_layers"],
    ...
    def optimize(
        ...
        search_algo = OptunaSearch(metric="sharpe_ratio", mode="max")
        ...
        analysis = tune.Tuner(
            ...
            run_config=air.RunConfig(
                storage_path="file:///tmp/ray_results", name="ppo_hyperopt"
            ),
        ).fit()
