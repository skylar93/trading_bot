...
class TrainingOptimizer:
    ...
    def start(self):
        ...
        self._running = True
        self._optimize_thread = threading.Thread(target=self._optimize_loop, daemon=True)
    ...
    def _apply_optimizations(
        ...
        if any("resource" in s.lower() for s in suggestions):
            ...
            elif worker_stats['avg_batch_time'] < self.config.target_batch_time * 0.5:
                # Can reduce workers
                current = worker_stats['active_workers']
    ...
    def _optimize_batch_size(self, analysis: Any) -> int:
        ...
        if len(self._optimization_history) >= 2:
            prev_throughput = self._optimization_history[-2]['metrics']['throughput']
            if analysis.throughput < prev_throughput * 0.9:
                # Significant throughput drop - revert changes
                return self._optimization_history[-2].get('batch_size', current)
