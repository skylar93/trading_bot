...
class DistributedTrainingManager:
                ...
                for epoch in range(self.config.training_config.num_epochs):
                    try:
                        ...
                        metrics = self._process_epoch(
                            train_data, val_data, batch_size, actor_ids
                        )
                        ...
                        logger.error(
                            f"Error in epoch {epoch}: {str(e)}", exc_info=True
                        )
                        continue
    ...
    def _initialize_actors(self, data: pd.DataFrame) -> list:
        ...
        for i in range(self.config.training_config.num_parallel):
            try:
                # Request resources
                allocation = ray.get(
                    self.resource_manager.allocate_resources.remote(
                        f"actor_{i}",
                        cpu_needed=1,
                        memory_needed=data.memory_usage().sum()
                        / self.config.training_config.num_parallel,
                        gpu_needed=(
                            1 if self.config.training_config.num_gpus else None
                        ),
                    )
                )
                ...
                logger.error(
                    f"Error initializing actor {i}: {str(e)}", exc_info=True
                )
    ...
    def _process_epoch(
        ...
        while unfinished_batches:
            ...
            for batch_index, batch in unfinished_batches:
                ...
                should_continue = ray.get(
                    self.recovery_manager.heartbeat.remote(
                        actor_id, {"batch_index": batch_index}
                    )
                )
                ...
                if should_continue:
                    ...
                    if checkpoint:
                        ...
                        futures[batch_index] = (
                            self.trainer.resume_batch.remote(
                                batch, checkpoint, actor_id
                            )
                        )
                    else:
                        # Retry from scratch
                        futures[batch_index] = (
                            self.trainer.process_batch.remote(batch, actor_id)
                        )

            # Wait for results with timeout
            ready_futures, pending = ray.wait(
                list(futures.values()), timeout=30.0  # 30 seconds timeout
            )
            ...
            for batch_index, future in futures.items():
                if future in ready_futures:
                    try:
                        ...
                        actor_id = actor_ids[batch_index % len(actor_ids)]
                        ray.get(
                            self.recovery_manager.save_checkpoint.remote(
                                actor_id, {"batch_result": result}
                            )
                        )
                        ...
                        actor_id = actor_ids[batch_index % len(actor_ids)]
                        ray.get(
                            self.recovery_manager.mark_worker_failed.remote(
                                actor_id
                            )
                        )
            ...
            unfinished_batches = [
                (i, b)
                for i, b in unfinished_batches
                if i not in completed_indices
            ]

        # Check for failed workers
        failed_workers = ray.get(
            self.recovery_manager.get_failed_workers.remote()
        )
        if failed_workers:
    ...
    def _save_checkpoint(self, epoch: int):
        ...
        checkpoint_path = os.path.join(
            self.config.checkpoint_dir, f"checkpoint_epoch_{epoch}"
        )
    ...
    def cleanup(self):
        ...
        try:
            ...
            for actor_id in actor_states:
                ray.get(
                    self.resource_manager.release_resources.remote(actor_id)
                )
