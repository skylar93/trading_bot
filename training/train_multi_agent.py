...
def train_multi_agent_system(
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            next_observations, rewards, dones, truncated, infos = env.step(
                actions
            )
        ...
        for agent_id, agent in agents.items():
            if len(replay_buffers[agent_id]) > 0:
                ...
                # Update metrics
                metrics[agent_id]["episode_rewards"].append(
                    episode_rewards[agent_id]
                )
                metrics[agent_id]["portfolio_values"].append(
                    infos[agent_id]["portfolio_value"]
                )
                metrics[agent_id]["policy_losses"].append(
                    loss_info["policy_loss"]
                )
                metrics[agent_id]["value_losses"].append(
                ...
                mlflow.log_metrics(
                    {
                        ...
                        f"{agent_id}_portfolio_value": infos[agent_id][
                            "portfolio_value"
                        ],
            ...
            for agent_id, agent in agents.items():
                try:
                    save_file = os.path.join(
                        save_path, f"{agent_id}_episode_{episode}.pt"
                    )
                    agent.save(save_file)
                    logger.info(
                        f"Saved model for agent {agent_id} at episode {episode}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to save model for agent {agent_id}: {str(e)}"
                    )
        ...
        if episode % 10 == 0:
            ...
            for agent_id in agents.keys():
                ...
                logger.info(
                    f"Portfolio Value: {infos[agent_id]['portfolio_value']:.2f}"
                )
                if agent_id in loss_infos:
                    logger.info(
                        f"Policy Loss: {loss_infos[agent_id]['policy_loss']:.4f}"
                    )
                    logger.info(
                        f"Value Loss: {loss_infos[agent_id]['value_loss']:.4f}"
                    )
            logger.info("\n-------------------")
