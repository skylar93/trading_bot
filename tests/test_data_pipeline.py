...
def test_feature_generator(sample_data):
    ...
    assert (df_features['RSI'] >= 0).all() and (df_features['RSI'] <= 100).all(), "RSI values out of range"
    assert (df_features['BB_upper'] >= df_features['BB_lower']).all(), "Invalid Bollinger Bands"
    ...
    for window in [5, 10, 20]:
        ...
        assert (df_features[f'SMA_{window}'] >= price_min).all() and (df_features[f'SMA_{window}'] <= price_max).all(), \
            f"SMA_{window} values out of price range"
        assert (df_features[f'EMA_{window}'] >= price_min).all() and (df_features[f'EMA_{window}'] <= price_max).all(), \
            f"EMA_{window} values out of price range"
...
def test_feature_generator_error_handling(empty_df, invalid_df, nan_df):
    ...
    assert not features_with_nan.isnull().any().any(), "NaN values should be handled"
...
def test_simple_pipeline():
    ...
    assert len(df_features.columns) > len(df.columns), "No new features generated"
