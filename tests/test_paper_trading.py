...
async def test_position_sizing(paper_env):
    ...
    try:
        ...
        assert 0 <= size <= paper_env.initial_balance * paper_env.max_position_size
    finally:
        await paper_env.cleanup()
...
async def test_risk_management(paper_env):
    ...
    try:
        ...
        await paper_env.update_market_data({
            'type': 'ticker',
            'data': {'close': paper_env.stop_loss_price * 0.99}  # Price below stop loss
        })
