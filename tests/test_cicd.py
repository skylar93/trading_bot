...
def test_data_pipeline():
    ...
    # Verify features
    assert len(features.columns) > len(data.columns), "Should generate additional features"
...
def test_full_pipeline():
    ...
    logger.debug(f"Split data into {len(train_data)} training and {len(val_data)} validation samples")
    ...
    config = {
        ...
        'training': {
            'total_timesteps': 50  # Minimal steps
        }
    }
    ...
    try:
        agent = train_agent(train_data=train_data, val_data=val_data, config=config)
    ...
    try:
        ...
        while not (done or truncated) and total_steps < max_steps:
            ...
            total_reward += reward
            total_steps += 1
            logger.debug(f"Step {total_steps} reward: {reward:.2f}, total reward: {total_reward:.2f}")
