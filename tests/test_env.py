...
def sample_env_data():
    ...
    data["$high"] = np.maximum(data["$open"], data["$close"]) + abs(
        np.random.normal(0, 0.5, len(dates))
    )
    data["$low"] = np.minimum(data["$open"], data["$close"]) - abs(
        np.random.normal(0, 0.5, len(dates))
    )
...
class TestTradingEnvironment:
    ...
    def test_step_buy_action(self, trading_env):
        ...
        obs, reward, terminated, truncated, info = trading_env.step(
            np.array([1.0])
        )  # Full buy
...
class TestEnvironmentWrappers:
    ...
    def test_full_wrapped_env(self, trading_env):
        ...
        wrapped_env = make_env(trading_env, normalize=True, stack_size=4)
